...
class PersonDAO extends DAO {
    ...
    async insert(Person: IPerson) {
        try {
            await postgreSQL.query('INSERT INTO ' + PersonDAO.table + '(name_pers, cpf_pers, phone_pers, address_pers, num_address, fk_name_filial, fk_id_user, bairro_pers, fk_cep, rg, cnpj, inscricao, fantasia, limit_cred, fk_grupo) VALUES (' + "'" + Person.name_pers + "', '" + Person.cpf_pers + "', '" + Person.phone_pers + "', '" + Person.address_pers + "', '" + Person.num_address + "', '" + Person.fk_name_filial + "', '" + Person.fk_id_user + "', '" + Person.bairro_pers + "', '" + Person.fk_cep + "', '" + Person.rg + "', '" + Person.cnpj + "', '" + Person.inscricao + "', '" + Person.fantasia + "', '" + Person.limit_cred + "', '" + Person.fk_grupo + "')")
        } catch (err) {
    ...
    async update(Person: IPerson) {
        try {
            await postgreSQL.query("UPDATE " + PersonDAO.table + " SET updated_at =  now(), name_pers = '" + Person.name_pers + "', cpf_pers = '" + Person.cpf_pers + "', phone_pers ='" + Person.phone_pers + "', address_pers ='" + Person.address_pers + "', num_address = '" + Person.num_address + "', bairro_pers = '" + Person.bairro_pers + "', fk_cep = '" + Person.fk_cep + "', fk_name_filial = '" + Person.fk_name_filial + "', rg = '" + Person.rg + "', cnpj = '" + Person.cnpj + "', inscricao = '" + Person.inscricao + "', fantasia = '" + Person.fantasia + "', limit_cred = '" + Person.limit_cred + "', fk_grupo = '" + Person.fk_grupo + "' WHERE id_person = '" + Person.id_person + "'")
        } catch (err) {
            return (new PersonDAO().errors(err))
        }
    };
